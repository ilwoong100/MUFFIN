...
class SequentialRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        if config['DOMAIN_ID_FIELD'] is not None:
            self.DOMAIN_ID = config['DOMAIN_ID_FIELD']
            self.POS_DOMAIN_ID = self.DOMAIN_ID
            self.DOMAIN_SEQ = self.DOMAIN_ID + config['LIST_SUFFIX']
            self.DOMAIN_SEQ_LEN = config['ITEM_LIST_LENGTH_FIELD']
            self.n_domains = dataset.num(self.DOMAIN_ID)
