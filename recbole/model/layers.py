...
class TransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        self.feed_forward = FeedForward(hidden_size, intermediate_size, hidden_dropout_prob, hidden_act, layer_norm_eps, config)
...
class TFAEncoder(nn.Module):
    def __init__(self, config, n_layers, n_heads, hidden_size, inner_size, hidden_dropout_prob, freq_dropout_prob, hidden_act, layer_norm_eps):
        super().__init__()
        self.layer = nn.ModuleList()
        for n in range(n_layers):
            self.tfablock = TFABlock(config, hidden_size, inner_size, hidden_dropout_prob, hidden_act, freq_dropout_prob, layer_norm_eps)
            self.layer.append(self.tfablock)
    
    def forward(self, hidden_states, item_seq_len,timestamp, global_mod, output_all_encoded_layers):
        all_encoder_layers = []
        for layer_module in self.layer:
            hidden_states, local_mask = layer_module(hidden_states,item_seq_len, timestamp, global_mod)
            if output_all_encoded_layers:
                all_encoder_layers.append(hidden_states)
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)

        return all_encoder_layers, local_mask
        

class TFABlock(nn.Module):
    def __init__(self, config, hidden_size, inner_size, hidden_dropout_prob, hidden_act, freq_dropout_prob, layer_norm_eps):
        super().__init__()
        # self.ndft_layer = NDFTLayer(config, hidden_size, freq_dropout_prob, )
        if config['NDFT']:
            self.hybrid_filter_layer = NDFTHybridLayer(config)
        else:
            self.hybrid_filter_layer = HybridLayer(config)
        self.feed_forward = FeedForward(hidden_size, inner_size, hidden_dropout_prob, hidden_act, layer_norm_eps)
    
    def forward(self, hidden_states, item_seq_len, timestamp, global_mod):
        hybrid_output, local_mask = self.hybrid_filter_layer(hidden_states, item_seq_len, timestamp, global_mod)
        feedforward_out =  self.feed_forward(hybrid_output)
        return feedforward_out, local_mask
...
class HybridLayer(nn.Module):
    def __init__(self, config):
        ...
        # self.threshold =  nn.Parameter(torch.rand(config['hidden_size']))
        self.local_weight = nn.Parameter(torch.randn(1, config['MAX_ITEM_LIST_LENGTH']//2 + 1, config['hidden_size'], 2, dtype=torch.float32) * 0.02)
        self.global_weight = nn.Parameter(torch.randn(1, config['MAX_ITEM_LIST_LENGTH']//2 + 1, config['hidden_size'], 2, dtype=torch.float32) * 0.02)
        self.softness=config['softness']
        self.local_mod = config['local_mod']

    def forward(self, input_tensor,item_seq_len, timestamp, global_mod=False):
        ...
        if global_mod==True:
            local_mask=None
            global_weight = torch.view_as_complex(self.global_weight)
            global_x = x * global_weight
            sequence_emb_fft = torch.fft.irfft(global_x, n=seq_len, dim=1, norm='ortho')    
            hidden_states = self.out_dropout(sequence_emb_fft)
            hidden_states = self.LayerNorm(hidden_states + input_tensor)
        elif self.local_mod==True:
            local_mask = self.local_mask(x)
            local_x = local_mask * x
            sequence_emb_fft = torch.fft.irfft(local_x, n=seq_len, dim=1, norm='ortho')

        else:
            
            local_mask = self.local_mask(x)
            local_x = local_mask * x
            local_weight = torch.view_as_complex(self.local_weight)
            global_weight = torch.view_as_complex(self.global_weight)
            local_x = local_x * local_weight
            global_x = x * global_weight

            hybrid = local_x + global_x
            
            sequence_emb_fft = torch.fft.irfft(hybrid, n=seq_len, dim=1, norm='ortho')    

            hidden_states = self.out_dropout(sequence_emb_fft)
            hidden_states = self.LayerNorm(hidden_states + input_tensor)

        return hidden_states, local_mask

    # def local_mask(self, fft_seq):
    #     amp = torch.abs(fft_seq)
    #     amp_mean = amp.mean(dim=1, keepdim=True)
    #     amp_std = amp.std(dim=1, keepdim=True)
    #     normalized_amp = (amp - amp_mean) / (amp_std + 1e-6)
    #     soft_threshold = torch.sigmoid((normalized_amp * self.softness - self.threshold) )  
    #     self.soft_threshold =  soft_threshold
    #     return soft_threshold
    
    def local_mask(self, fft_seq):
        self.threshold = torch.rand(fft_seq.shape[-1]).to(fft_seq.device)
        amp = torch.abs(fft_seq)
        amp_median = amp.median(dim=1, keepdim=True)[0]
        normalized_amp = amp / (amp_median + 1e-6)
        high_amplitude_mask = ((normalized_amp > self.threshold).float() - self.threshold).detach() + self.threshold
        
        return high_amplitude_mask
        
class NDFTHybridLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.out_dropout = nn.Dropout(config['freq_dropout_prob'])
        self.LayerNorm = nn.LayerNorm(config['hidden_size'], eps=1e-12)
        self.threshold =  nn.Parameter(torch.rand((config['hidden_size'],1)))
        self.N = config['N']
        local_random_complex = torch.randn(1, config['hidden_size'], self.N//2+1, 2, dtype=torch.float32) * 0.02
        local_second_half = torch.flip(torch.cat((local_random_complex[..., 0:1], -local_random_complex[..., 1:2]), dim=-1),dims=[2])[:,:,1:-1]
        self.local_complex_weight = nn.Parameter(torch.cat((local_random_complex, local_second_half), dim=2))

        global_random_complex = torch.randn(1, config['hidden_size'], self.N//2+1, 2, dtype=torch.float32) * 0.02
        global_second_half = torch.flip(torch.cat((global_random_complex[..., 0:1], -global_random_complex[..., 1:2]), dim=-1),dims=[2])[:,:,1:-1]
        self.global_complex_weight = nn.Parameter(torch.cat((global_random_complex, global_second_half), dim=2))
    ...
    def forward(self, input_tensor,item_seq_len,timestamp, global_mod=False):
        ...
        x = self.ndft_seq(timestamp, input_tensor.permute(0,2,1), self.N)
        ...
        if global_mod==True:
            ...
            local_weight = torch.view_as_complex(self.local_complex_weight)
            global_weight = torch.view_as_complex(self.global_complex_weight)
            local_x = local_x * local_weight
            global_x = x * global_weight
            hybrid = local_x + global_x
            sequence_emb_ndft = self.ndft_adjoint_seq(timestamp, hybrid, self.N).real
            hidden_states = self.out_dropout(sequence_emb_ndft)
            hidden_states = self.LayerNorm(hidden_states + input_tensor)

        return hidden_states, local_mask

    
    def local_mask(self, ndft_seq):
        amp = torch.abs(ndft_seq)
        amp_median = amp.median(dim=2, keepdim=True)[0]
        normalized_amp = amp / (amp_median + 1e-6)
        high_amplitude_mask = ((normalized_amp > self.threshold).float() - self.threshold).detach() + self.threshold
        return high_amplitude_mask
        
    def ndft_seq(self,x, signal, N):
        """Non-equispaced discrete Fourier transform"""
        k = -(N // 2) + torch.arange(N).to(signal.device)
        signal = torch.complex(real=signal, imag=torch.zeros_like(signal))
        exp_coeff = torch.exp(2j * torch.pi * k * x.unsqueeze(-1)).conj()
        freq = torch.matmul(signal, exp_coeff)
        return freq / (N ** 0.5)
    
    def ndft_adjoint_seq(self, x, freq_data,N):
        """Non-equispaced discrete Fourier transform adjoint (inverse)"""
        k = -(N // 2) + torch.arange(N).to(freq_data.device)
        exp_coeff = torch.exp(-2j * torch.pi *  x.unsqueeze(1) * k[:,None]).conj()
        signal = torch.matmul(freq_data, exp_coeff)
        return signal.permute(0, 2, 1) / (N **0.5)
    
class NDFTLayer(nn.Module):
    def __init__(self, config, hidden_size, freq_dropout_prob):
        super(NDFTLayer, self).__init__()
        self.out_dropout = nn.Dropout(freq_dropout_prob)
        self.LayerNorm = nn.LayerNorm(hidden_size, eps=1e-12)
        self.N = config['N']
        random_complex = torch.randn(1, hidden_size, self.N//2+1, 2, dtype=torch.float32) * 0.02
        second_half = torch.flip(torch.cat((random_complex[..., 0:1], -random_complex[..., 1:2]), dim=-1),dims=[2])[:,:,1:-1]
        self.complex_weight = nn.Parameter(torch.cat((random_complex, second_half), dim=2))

    def forward(self, input_tensor, item_seq_len, timestamp):
        # [batch, seq_len, hidden]
        batch, max_len, hidden = input_tensor.shape

        x = self.ndft_seq(timestamp, input_tensor.permute(0,2,1), self.N, max_len)
        
        weight = torch.view_as_complex(self.complex_weight)
        x = x * weight

        sequence_emb_ndft = self.ndft_adjoint_seq(timestamp, x, self.N,max_len).real # * mask.unsqueeze(-1)
    ...
    def ndft_seq(self,x, signal, N, max_len):
        """Non-equispaced discrete Fourier transform"""
        k = -(N // 2) + torch.arange(N).to(signal.device)
        signal = torch.complex(real=signal, imag=torch.zeros_like(signal))
        exp_coeff = torch.exp(2j * torch.pi * k * x.unsqueeze(-1)).conj()
    ...
    def ndft_adjoint_seq(self, x, freq_data,N, max_len):
        """Non-equispaced discrete Fourier transform adjoint (inverse)"""
        k = -(N // 2) + torch.arange(N).to(freq_data.device)
        exp_coeff = torch.exp(-2j * torch.pi *  x.unsqueeze(1) * k[:,None]).conj()
        signal = torch.matmul(freq_data, exp_coeff)
        ...
        # low_freq = x[:].clone()
        # low_freq[:,:,int(self.N//2 + self.c)+1:] = 0
        # low_freq[:,:,1:int(self.N//2 - self.c)] = 0
        
        # fft사용을 위해서는 해당 작업이 반드시 필요함, 홀수번째 열에 -1을 곱해 주어야만 함
        # x[:,:,1::2] *= -1
        
        # fft_result = torch.fft.rfft(input_tensor,dim=1, norm='ortho')
        # low_pass = fft_result[:].clone()
        # low_pass[:, self.c:, :] = 0
        # fft_recon = torch.fft.irfft(low_pass, n=max_len, dim=1, norm='ortho')   


        # high_freq = x[:].clone()
        # high_freq[:,:,int(self.N//2 - self.N//2*self.tmp):int(self.N//2 + self.N//2 *self.tmp)+1] = 0
        # low_freq = low_freq[:,:,int(self.N//2):int(self.N//2+ max_len//2 +1)]
        # low_freq[:,:,self.c:] = 0 
        
        # from IPython import embed
        # embed()
        
        # freqs = torch.fft.fftshift(torch.fft.fftfreq(x.size(2), d=1/self.N).to(input_tensor.device))
        # seq_len_norm = seq_len * (self.N//2) 
        # seq_len_norm_dim = seq_len_norm.unsqueeze(-1).unsqueeze(-1)

        # # low_cutoffs = torch.floor(seq_len_norm_dim * self.tmp)
        # low_cutoffs = torch.floor(seq_len_norm_dim )
        # # high_cutoffs = torch.floor(seq_len_norm_dim)
        # low_mask = torch.abs(freqs ) <= low_cutoffs
        # high_mask = torch.abs(freqs ) <= high_cutoffs
        # high_mask = high_mask ^ low_mask

        
        # low_freq = x[:].clone() * low_mask

        # weight = torch.view_as_complex(self.complex_weight)
        # low_freq = low_freq * weight
        # high_freq = x[:].clone() * high_mask

        # low_signal = torch.fft.irfft(low_freq.permute(0,2,1), n=50, dim=1, norm='ortho')
        # high_freq = x - low_freq
        # low_freq = low_freq * weight

        # high_signal = self.ndft_adjoint_seq(timestamp, high_freq, self.N,max_len).real # * mask.unsqueeze(-1)                

        # high_signal = self.ndft_adjoint_seq(timestamp, high_freq, self.N).real  * mask.unsqueeze(-1)
