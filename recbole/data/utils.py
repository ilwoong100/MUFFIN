...
def data_preparation_head_tail(config, dataset, save=False, verbose=False):
    """Split the dataset by config settings and create train/valid/test plus head/tail test dataloaders."""
    # Build raw datasets
    es = EvalSetting(config)
    train_dataset, valid_dataset, test_dataset = dataset.build(es)

    # Determine saved dataloaders path
    save_path = config['checkpoint_dir']
    saved_name = f"{config['dataset']}-for-{config['model']}-dataloader.pth"
    file_path = os.path.join(save_path, saved_name)
    existing = None
    if os.path.exists(file_path):
        try:
            existing = load_split_dataloaders(file_path)
        except Exception:
            existing = None

    # Load or build the first three dataloaders
    if existing and len(existing) >= 3:
        train_data, valid_data, test_data = existing[:3]
    else:
        phases = ['train', 'valid', 'test']
        sampler = None

        # --- Training loader ---
        train_neg = config['train_neg_sample_args']
        train_kwargs = {
            'config': config,
            'dataset': train_dataset,
            'batch_size': config['train_batch_size'],
        ...
        if train_neg['strategy'] != 'none':
            if config['MODEL_TYPE'] != ModelType.SEQUENTIAL:
                sampler = Sampler(phases, (train_dataset, valid_dataset, test_dataset), train_neg['distribution'])
            else:
                sampler = RepeatableSampler(phases, train_dataset, train_neg['distribution'])
            train_kwargs['sampler'] = sampler.set_phase('train')
            train_kwargs['neg_sample_args'] = train_neg
            if config['MODEL_TYPE'] == ModelType.KNOWLEDGE:
                train_kwargs['kg_sampler'] = KGSampler(train_dataset, train_neg['distribution'])
        TrainLoader = get_data_loader('train', config, train_neg)
        train_data = TrainLoader(**train_kwargs)

        # --- Validation & Test loaders ---
        eval_neg = es.neg_sample_args
        base_kwargs = {
            'config': config,
            'batch_size': config['eval_batch_size'],
            'dl_format': getattr(es, 'MODEL_INPUT_TYPE', None) or 'pointwise',
            'shuffle': False,
            'phase': 'eval'
        }
        valid_kwargs = {'dataset': valid_dataset}
        test_kwargs = {'dataset': test_dataset}
        if eval_neg['strategy'] != 'none':
            if sampler is None:
                if config['MODEL_TYPE'] != ModelType.SEQUENTIAL:
                    sampler = Sampler(phases, (train_dataset, valid_dataset, test_dataset), eval_neg['distribution'])
                else:
                    sampler = RepeatableSampler(phases, train_dataset, eval_neg['distribution'])
            sampler.set_distribution(eval_neg['distribution'])
            valid_kwargs.update({'sampler': sampler.set_phase('valid'), 'neg_sample_args': eval_neg})
            test_kwargs.update({'sampler': sampler.set_phase('test'), 'neg_sample_args': eval_neg})
        valid_kwargs.update(base_kwargs)
        test_kwargs.update(base_kwargs)
        EvalLoader = get_data_loader('evaluation', config, eval_neg)
        valid_data = EvalLoader(**valid_kwargs)
        test_data  = EvalLoader(**test_kwargs)

        # Save if needed
        if save or config['save_dataloaders']:
            save_split_dataloaders(config, (train_data, valid_data, test_data))

    # --- Head/Tail split for test ---
    # Compute item popularity from train set
    _, item_pop = train_dataset.get_user_item_pop()
    sorted_idx = np.argsort(item_pop)
    cutoff = int(len(sorted_idx) * 0.2)
    head_items = set(sorted_idx[-cutoff:])
    tail_items = set(sorted_idx[:-cutoff])

    # Filter test interactions
    inter_feat = test_dataset.inter_feat
    head_mask = np.isin(inter_feat['item_id'], list(head_items))
    tail_mask = np.isin(inter_feat['item_id'], list(tail_items))

    head_ds = copy.deepcopy(test_dataset)
    tail_ds = copy.deepcopy(test_dataset)
    head_ds.inter_feat = inter_feat[head_mask]
    tail_ds.inter_feat = inter_feat[tail_mask]

    # Build head & tail test loaders
    head_kwargs = dict(
        config=config,
        dataset=head_ds,
        batch_size=config['eval_batch_size'],
        dl_format=base_kwargs['dl_format'],
        shuffle=False,
        phase='eval'
    )
    tail_kwargs = dict(head_kwargs)
    if eval_neg['strategy'] != 'none':
        head_kwargs.update({'sampler': test_kwargs.get('sampler'), 'neg_sample_args': eval_neg})
        tail_kwargs.update({'sampler': test_kwargs.get('sampler'), 'neg_sample_args': eval_neg})

    head_test_data = get_data_loader(config,**head_kwargs)
    tail_test_data = EvalLoader(**tail_kwargs)

    # Save all five if needed
    if save or config['save_dataloaders']:
        save_split_dataloaders(config, (train_data, valid_data, test_data, head_test_data, tail_test_data))

    # Verbose logging
    if verbose:
        logger = getLogger()
        logger.info(set_color('[Training]:', 'pink') + f' train batch size = {config["train_batch_size"]}')
        logger.info(set_color('[Evaluation]:', 'pink') + f' eval batch size = {config["eval_batch_size"]}')
